...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  private readonly socketRoom = new Map<string, string>();   // socketId → roomCode
  ...
  async handleMove(
    ...
    if (this.checkWin(room.pieces, myRole)) {
      room.wins[myRole]++;
      await this.handleWin(room, myRole); // ← await corrigido
    } else {
  ...
  async resetGame(@ConnectedSocket() client: Socket) {
    ...
    room.turn = 'p1';
    this.server.to(roomCode).emit('gameState', this.roomPayload(room));
    this.server.to(roomCode).emit('gameReset');
  }
  ...
  private emitGameStart(room: RoomState) {
    ...
    // cada cliente recebe seu role individualmente
  ...
  private leaveRoom(client: Socket) {
    ...
    if (room) {
      ...
      // limpa TODOS os jogadores da sala (corrigido)
    ...
    // garante limpeza do próprio client caso não estivesse em room
